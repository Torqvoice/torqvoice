...
const attachment = (
  overrides: Partial<{ filename: string; contentType: string; size: number }> = {}
) => ({
...
describe('validateSupportRequest', () => {
  ...
  it('rejects text beyond the stated limits', () => {
    expect(
      validateSupportRequest(request({ subject: 'x'.repeat(MAX_SUBJECT_LENGTH + 1) }))
    ).toEqual({
      ok: false,
      reason: 'subject-too-long',
    })
    expect(
      validateSupportRequest(request({ message: 'x'.repeat(MAX_MESSAGE_LENGTH + 1) }))
    ).toEqual({
      ok: false,
  ...
  it('rejects file types that have no business in an inbox', () => {
    for (const contentType of [
      ...
      expect(
        validateSupportRequest(request({ attachments: [attachment({ contentType })] }))
      ).toEqual({
        ok: false,
  ...
  it('accepts every type on the allow list', () => {
    for (const contentType of ALLOWED_ATTACHMENT_TYPES) {
      expect(
        validateSupportRequest(request({ attachments: [attachment({ contentType })] })).ok
      ).toBe(true)
    }
  })
  ...
  it('offers the picker exactly the types the server accepts', () => {
    ...
    for (const offered of ATTACHMENT_ACCEPT.split(',')) {
      expect(
        validateSupportRequest(request({ attachments: [attachment({ contentType: offered })] })).ok
      ).toBe(true)
    }
