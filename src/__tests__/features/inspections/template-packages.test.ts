...
describe('package format', () => {
  ...
  it('builds a filename that survives a downloads folder', () => {
    expect(packageFileName({ name: 'Norway — EU-kontroll' })).toBe(
      'torqvoice-norway-eu-kontroll.json'
    )
...
describe('package registry', () => {
  ...
  it('refuses a content type this version cannot install', () => {
    ...
    expect(() => reviewContents([{ type: 'labour-preset', data: {} }])).toThrow(/cannot install/i)
  })
