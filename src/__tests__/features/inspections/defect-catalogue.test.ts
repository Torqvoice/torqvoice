...
import { conditionGrade, gradedConditionLabel } from '@/features/inspections/Lib/conditions'
...
describe('national defect codes', () => {
  ...
  it('puts the code before the wording', () => {
    ...
    expect(gradedConditionLabel('attention', 'eu', 'NO', 'Mindre mangel')).toBe('1 — Mindre mangel')
  })
})
...
describe('rankSuggestions', () => {
  ...
  it('carries the severity the Directive assigns to each reason', () => {
    ...
    const metalToMetal = suggestions.find((s) => s.text.includes('metal to metal contact'))
  ...
  it('does not repeat a phrase that appears in more than one source', () => {
    const suggestions = rankSuggestions(BRAKE_PADS, {
      history: [{ text: 'Lining or pad missing or incorrectly mounted', severity: 'fail' }],
    })
