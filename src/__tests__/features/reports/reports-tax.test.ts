...
describe('getRevenueReport — net parts/labor calculation', () => {
  ...
  it('the same logical revenue produces identical netProfit in both modes', async () => {
    ...
    expect(exclusive.data?.summary.totalPartsRevenue).toBeCloseTo(
      inclusive.data!.summary.totalPartsRevenue
    )
    expect(exclusive.data?.summary.totalLaborRevenue).toBeCloseTo(
      inclusive.data!.summary.totalLaborRevenue
    )
  })
})
