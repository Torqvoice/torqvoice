...
describe('createServiceRecord — parts and inventory', () => {
  it('creates part items in the transaction', async () => {
    ...
    await createServiceRecord({
      ...
      title: 'Brake Job',
      partItems: [{ name: 'Brake Pads', quantity: 2, unitPrice: 45, total: 90 }],
      subtotal: 90,
      taxRate: 0,
      taxAmount: 0,
      totalAmount: 90,
  ...
  it('deducts inventory stock when part has inventoryPartId', async () => {
    ...
    const mockInvFind = vi
      .fn()
      .mockResolvedValue({ id: 'inv-1', quantity: 10, organizationId: ORG })
