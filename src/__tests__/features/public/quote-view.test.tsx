    ...
    it('renders parts table when parts are present', () => {
      const props = {
        ...
        quote: {
          ...BASE_QUOTE,
          partItems: [
            { partNumber: 'P-001', name: 'Oil Filter', quantity: 1, unitPrice: 15, total: 15 },
          ],
        },
      }
      render(<QuoteView {...props} />)
    ...
    it('calls /api/public/forms/quote-response with action=accepted', async () => {
      ...
      await waitFor(() => {
        ...
        const call = mockFetch.mock.calls.find(
          (c: unknown[]) => c[0] === '/api/public/forms/quote-response'
        )
    ...
    it('the same logical quote produces identical visible totals in both modes', () => {
      ...
      const exclusiveProps = {
        ...
        quote: {
          ...
          taxInclusive: false,
          partItems: [
            { name: 'Brake Pad', partNumber: 'BP-001', quantity: 1, unitPrice: 100, total: 100 },
          ],
          subtotal: 100,
          taxAmount: 25,
          totalAmount: 125,
