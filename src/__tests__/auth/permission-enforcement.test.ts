...
describe('Permission denial — restricted custom role with no permissions', () => {
  ...
  const denialCases: [string, string, () => Promise<any>][] = [
    ...
    // The tire hotel holds customers' property and bills them for it, so its
    // own subject has to deny just as firmly as the rest.
    ['TIRE_HOTEL', 'READ', () => getTireSetsPaginated({})],
    ['TIRE_HOTEL', 'READ', () => getSetsForForecast()],
  ...
  it.each(
    denialCases
  )('denies access to %s (%s) when user has no permissions', async (_subject, _action, callAction) => {
...
describe('Permission grant — correct permission allows access past the gate', () => {
  const grantCases: [string, { action: string; subject: string }, () => Promise<any>][] = [
    ...
    [
      'INSPECTIONS (READ)',
      { action: 'read', subject: 'inspections' },
    ...
    [
      ...
      () => getSetting('organization.name' as any),
    ],
    ['SETTINGS (MANAGE)', { action: 'manage', subject: 'settings' }, () => removeMember('some-id')],
    [
      'TIRE_HOTEL (READ)',
      { action: 'read', subject: 'tire_hotel' },
      () => getTireSetsPaginated({}),
    ],
    ['TIRE_HOTEL (CREATE)', { action: 'create', subject: 'tire_hotel' }, () => checkInTireSet({})],
    ['TIRE_HOTEL (UPDATE)', { action: 'update', subject: 'tire_hotel' }, () => disposeTireSet({})],
  ]

  it.each(
    grantCases
  )('grants access to %s when user has the correct permission', async (_label, permission, callAction) => {
...
describe('Unauthenticated access — all actions require authentication', () => {
  ...
  it.each(unauthCases)('%s rejects unauthenticated calls', async (_name, callAction) => {
    const result = await callAction()
    expect(result).toEqual(expect.objectContaining({ success: false, error: 'Unauthorized' }))
  })
})
