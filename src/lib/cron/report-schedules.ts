...
async function fetchRevenue(orgId: string, start: Date, end: Date) {
  ...
  return {
    monthly: Object.entries(monthly).map(([month, d]) => ({
      month,
      ...d,
      partsNetProfit: d.partsRevenue - d.partsCost,
      netProfit: d.partsRevenue - d.partsCost + d.laborRevenue,
    })),
...
async function fetchTechnicians(orgId: string, start: Date, end: Date) {
  ...
  const byTech: Record<
    string,
    { techName: string; jobCount: number; totalRevenue: number; totalLaborHours: number }
  > = {}
  for (const r of records) {
  ...
  const technicians = Object.values(byTech)
    .map((d) => ({
      ...
      totalLaborHours: d.totalLaborHours,
      avgHours: d.jobCount > 0 ? d.totalLaborHours / d.jobCount : 0,
    }))
...
async function fetchParts(orgId: string, start: Date, end: Date) {
  ...
  const result = Object.values(byPart)
    .map((d) => ({ ...d, netProfit: d.totalRevenue - d.totalCost }))
...
async function fetchJobAnalytics(orgId: string, start: Date, end: Date) {
  ...
  return {
    avgJobValue: records.length > 0 ? totalValue / records.length : 0,
    totalJobs: records.length,
    topServiceTypes: Object.entries(byType)
      .map(([type, d]) => ({
        type,
        count: d.count,
...
async function fetchRetention(orgId: string, start: Date, end: Date) {
  ...
  for (const c of customers) {
    ...
    for (const v of c.vehicles)
      for (const sr of v.serviceRecords)
        visits.push({
          date: sr.startDateTime ?? sr.serviceDate,
          amount: sr.totalAmount > 0 ? sr.totalAmount : sr.cost,
        })
    ...
    if (visits.length > 1) {
      ...
      for (let i = 1; i < visits.length; i++)
        gaps.push((visits[i].date.getTime() - visits[i - 1].date.getTime()) / 86400000)
      ...
      top.push({
        ...
        avgTimeBetweenVisits: Math.round(avg),
      })
    } else {
      newC++
    }
  ...
  return {
    ...
    newCustomers: newC,
    totalActive: returning + newC,
    avgTimeBetweenVisits:
      allGaps.length > 0 ? Math.round(allGaps.reduce((s, v) => s + v, 0) / allGaps.length) : null,
    topReturning: top.slice(0, 20),
  }
}
...
async function fetchInventory(orgId: string) {
  ...
  for (const p of parts) {
    ...
    if (p.minQuantity && p.quantity <= p.minQuantity) lowStock.push(p)
  }
...
async function fetchPastDue(orgId: string) {
  ...
  for (const r of records) {
    ...
    invoices.push({
      ...
      customerName: (r.customer ?? r.vehicle?.customer)?.name ?? 'Unknown',
      customerCompany: (r.customer ?? r.vehicle?.customer)?.company ?? null,
      vehicleInfo:
        (r.vehicle
...
export async function processOneSchedule(schedule: {
  ...
  console.warn(
    `[cron] Report schedule ${schedule.id} processed, sent to ${recipients.length} recipients`
  )
}
