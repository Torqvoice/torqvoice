...
function readSettings(rows: { key: string; value: string }[]): OrgAlertSettings {
  ...
  const parsedInterval = Number(
    map.get(SETTING_KEYS.LOW_STOCK_ALERTS_EMAIL_MIN_INTERVAL_HOURS),
  );
  ...
  return {
    ...
    emailMinIntervalHours: Number.isFinite(parsedInterval) && parsedInterval > 0
      ? parsedInterval
      : DEFAULT_EMAIL_MIN_INTERVAL_HOURS,
    lastEmailAt:
      lastEmailAt && !Number.isNaN(lastEmailAt.getTime()) ? lastEmailAt : null,
    defaultThreshold:
...
async function sendDigest(
  ...
  const rows = parts
    .map((p) => `<li>${formatLowStockLine(p, defaultThreshold)}</li>`)
    .join("");
  const subject =
    parts.length === 1
...
export async function processOrgLowStock(organizationId: string, now = new Date()) {
  ...
  >`
    SELECT "id", "name", "partNumber", "quantity", "minQuantity", "lowStockAlertedAt"
