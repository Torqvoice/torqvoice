...
import { SAMPLE_PAYLOADS, buildSampleEnvelope } from '@/features/webhooks/Lib/samples'
      ...
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            />
            <Button variant="outline" size="sm" onClick={onShowDeliveries} disabled={isPending}>
            ...
            {!allEvents && (
              <div className="space-y-4 rounded-lg border p-3">
                {WEBHOOK_EVENT_GROUPS.map((g) => {
                  const allSelected = g.events.every((e) => events.includes(e as string))
                  return (
...
function DeliveriesSheet({ webhook, onClose }: { webhook: Webhook; onClose: () => void }) {
  const t = useTranslations('settings.webhooks')
  ...
  const counts = useMemo(() => {
    const c = { all: 0, success: 0, failed: 0, retrying: 0, pending: 0 } as Record<
      StatusFilter,
      number
    >
  ...
  return (
    <>
      ...
      {previewId && (
        <DeliveryPayloadDialog deliveryId={previewId} onClose={() => setPreviewId(null)} />
      )}
    </>
  )
}
...
function DeliveryPayloadDialog({
  ...
  const t = useTranslations('settings.webhooks')
  const [data, setData] = useState<Awaited<ReturnType<typeof getDeliveryPayload>>['data'] | null>(
    null
  )
  const [loading, setLoading] = useState(true)
  const [copied, setCopied] = useState(false)
  ...
  return (
    ...
    >
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>{data ? `${data.event} · ${data.status}` : t('loadingPayload')}</DialogTitle>
          <DialogDescription>
            {data ? t('attempt', { n: data.attempt, max: data.maxAttempts }) : ''}
          </DialogDescription>
        </DialogHeader>
            ...
            {(data.statusCode != null || formattedResponse || data.errorMessage) && (
              <div>
                ...
                <div className="mt-1 space-y-2">
                  ...
                  {data.errorMessage && (
                    <p className="break-all font-mono text-xs text-red-600">{data.errorMessage}</p>
                  )}
                  {formattedResponse && (
                    <pre className="overflow-x-auto rounded-md bg-muted/40 p-3 text-[11px] leading-relaxed">
                      {formattedResponse}
