                  ...
                  <div key="header">
                    {(() => {
                      const statusBadge = (extraClass?: string) => (
                        <span
                          className={`${extraClass || ''} inline-block rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize ${
                            status === 'accepted'
                          ...
                          <>
                            ...
                            <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                              ...
                              <div className="flex gap-3 text-sm text-gray-500">
                                ...
                                {validUntilDate && (
                                  <span>{t('validUntil', { date: validUntilDate })}</span>
                                )}
                              </div>
                            </div>
                          </>
                            ...
                            <div className="sm:text-right">
                              ...
                              {validUntilDate && (
                                <p className="text-sm text-gray-500">
                                  {t('validUntil', { date: validUntilDate })}
                                </p>
                              )}
                              {statusBadge('mt-1')}
                            </div>
                      ...
                      switch (fid) {
                        ...
                        case 'vin':
                          return quote.vehicle!.vin ? (
                            <p key={fid} className="text-sm text-gray-500">
                              {serviceType === 'marine'
                                ? `HIN: ${quote.vehicle!.vin}`
                                : t('vin', { vin: quote.vehicle!.vin })}
                            </p>
                          ) : null
                        case 'license_plate':
                          return quote.vehicle!.licensePlate ? (
                            <p key={fid} className="text-sm text-gray-500">
                              {serviceType === 'marine'
                                ? `Reg: ${quote.vehicle!.licensePlate}`
                                : t('plate', { plate: quote.vehicle!.licensePlate })}
                            </p>
                          ) : null
                        default:
                          return null
                        ...
                        <tbody className="divide-y">
                          {quote.partItems.map((p, i) => {
                            ...
                            return (
                              <tr
                                key={i}
                                className={p.excluded ? 'line-through text-gray-400' : ''}
                              >
                        ...
                        <tbody className="divide-y">
                          {quote.laborItems.map((l, i) => {
                            ...
                            return (
                              <tr
                                key={i}
                                className={l.excluded ? 'line-through text-gray-400' : ''}
                              >
                ...
                return (
                  <div key="totals" className="mt-6 ml-auto max-w-xs space-y-2">
                    {(() => {
                      ...
                      const laborTotalStored = quote.laborItems.reduce(
                        (sum, l) => (l.excluded ? sum : sum + l.total),
                        0
                      )
                      const partsTotalStored = quote.partItems.reduce(
                      ...
                      const partsTotal = netLineTotal(
                        partsTotalStored,
                        quote.taxRate,
                        quoteTaxInclusive
                      )
                      const laborTotal = netLineTotal(
                        laborTotalStored,
                        quote.taxRate,
                        quoteTaxInclusive
                      )
                        ...
                        <>
                          {quote.laborItems.length > 0 && (
                            <div className="flex justify-between text-sm">
                              ...
                              <span>
                                {formatCurrency(laborTotal, currencyCode, currencyFormat)}
                              </span>
                            </div>
                          )}
                          {quote.partItems.length > 0 && (
                            <div className="flex justify-between text-sm">
                              ...
                              <span>
                                {formatCurrency(partsTotal, currencyCode, currencyFormat)}
                              </span>
                            </div>
                          )}
                          {sub > 0 && (
                          ...
                          {disc > 0 && (
                            <div className="flex justify-between text-sm">
                              <span className="text-gray-500">
                                {quote.discountType === 'percentage'
                                  ? t('discountPercent', { percent: quote.discountValue })
                                  : t('discount')}
                              </span>
                              <span className="text-red-500">
                                {formatCurrency(-disc, currencyCode, currencyFormat)}
                              </span>
                            </div>
                          )}
                          {quote.taxRate > 0 && (
