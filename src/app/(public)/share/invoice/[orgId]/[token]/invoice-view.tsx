            ...
            switch (sectionId) {
                  ...
                  <div key="header">
                    {(() => {
                      const renderModernField = (fid: string) => {
                        switch (fid) {
                          case 'logo':
                            return effectiveShowLogo && logoUrl ? (
                              <img
                                key="logo"
                                src={logoUrl}
                                alt={shopName}
                                ...
                                style={{
                                  maxHeight: 64 * (logoSize / 100),
                                  maxWidth: 180 * (logoSize / 100),
                                }}
                              />
                            ) : null
                          case 'company_name':
                            return effectiveShowCompanyName ? (
                              <h2 key="cn" className="text-xl font-bold sm:text-2xl">
                                {shopName}
                              </h2>
                            ) : null
                          case 'company_address':
                            return workshop.address ? (
                              <p key="addr" className="mt-1 text-sm opacity-80">
                                {workshop.address}
                              </p>
                            ) : null
                          case 'company_phone':
                            return workshop.phone ? (
                              <p key="ph" className="text-sm opacity-70">
                                {t('tel', { phone: workshop.phone })}
                              </p>
                            ) : null
                          case 'company_email':
                            return workshop.email ? (
                              <p key="em" className="text-sm opacity-70">
                                {workshop.email}
                              </p>
                            ) : null
                      ...
                      const renderCompactField = (fid: string) => {
                        switch (fid) {
                          case 'logo':
                            return effectiveShowLogo && logoUrl ? (
                              <img
                                ...
                                style={{
                                  height: 48 * (logoSize / 100),
                                  width: 48 * (logoSize / 100),
                                }}
                              />
                            ) : null
                          case 'company_name':
                          ...
                          case 'company_address':
                            return workshop.address ? (
                              <p key="addr" className="text-sm text-gray-500">
                                {workshop.address}
                              </p>
                            ) : null
                          case 'company_phone':
                            return workshop.phone ? (
                              <p key="ph" className="text-sm text-gray-500">
                                {t('tel', { phone: workshop.phone })}
                              </p>
                            ) : null
                          case 'company_email':
                            return workshop.email ? (
                              <p key="em" className="text-sm text-gray-500">
                                {workshop.email}
                              </p>
                            ) : null
                          case 'company_org_number':
                      ...
                      const renderStandardField = (fid: string) => {
                        switch (fid) {
                          ...
                          case 'company_address':
                            return workshop.address ? (
                              <p key="addr" className="mt-1 text-sm text-gray-500">
                                {workshop.address}
                              </p>
                            ) : null
                          case 'company_phone':
                      ...
                      if (headerStyle === 'compact')
                        return (
                          ...
                          >
                            <div>{headerFieldOrder.map(renderCompactField)}</div>
                      ...
                      return (
                        ...
                        >
                          <div>{headerFieldOrder.map(renderStandardField)}</div>
                          <div className="sm:text-right">
                            {showTorqvoiceBranding && (
              ...
              case 'service': {
                const renderInfoCard = (sid: string) => {
                  if (sid === 'customer') {
                    ...
                    if (
                      !(
                        ...
                        show('customer_email') ||
                        show('customer_phone') ||
                        show('customer_address') ||
                        show('customer_company') ||
                        show('customer_tax_id')
                      )
                    )
                      return null
                    const fieldOrder = getOrderedFieldIds(vf, [
                      'customer_name',
                      'customer_company',
                      'customer_address',
                      'customer_email',
                    ...
                    const renderField = (fid: string) => {
                      ...
                      switch (fid) {
                        case 'customer_name':
                          return (
                            <p key={fid} className="font-semibold">
                              {c.name}
                            </p>
                          )
                        case 'customer_company':
                          return c.company ? (
                            <p key={fid} className="text-sm">
                              {c.company}
                            </p>
                          ) : null
                        case 'customer_address':
                          return c.address ? (
                            <p key={fid} className="text-sm text-gray-500">
                              {c.address}
                            </p>
                          ) : null
                        case 'customer_email':
                      ...
                      switch (fid) {
                        ...
                        case 'mileage':
                          return vehicle.mileage > 0 ? (
                            <p key={fid} className="text-sm text-gray-500">
                              {serviceType === 'marine'
                                ? `Engine Hours: ${vehicle.mileage.toLocaleString(locale)}`
                  ...
                  if (sid === 'service') {
                    ...
                    const show = (fid: string) => !vf || vf.has(fid)
                    if (!(show('service_title') || show('service_type') || show('tech_name')))
                      return null
                    const fieldOrder = getOrderedFieldIds(vf, [
                      'service_title',
                      'service_type',
                  ...
                  return (
                    <div key={`col-${sectionId}`} className="mt-6 grid gap-4 sm:grid-cols-2">
                      <div className="space-y-4">
                        {colGroup.left.map((id) => (
                          <React.Fragment key={id}>{renderInfoCard(id)}</React.Fragment>
                        ))}
                      </div>
                      <div className="space-y-4">
                        {colGroup.right.map((id) => (
                          <React.Fragment key={id}>{renderInfoCard(id)}</React.Fragment>
                        ))}
                      </div>
                    </div>
                      ...
                      <table className="w-full min-w-125 text-sm">
                        ...
                        <tbody className="divide-y">
                          {record.partItems.map((p, i) => {
                            const netUnitPrice = netLineTotal(
                              p.unitPrice,
                              record.taxRate,
                              recordTaxInclusive
                            )
                            const netLineValue = netLineTotal(
                              p.total,
                              record.taxRate,
                              recordTaxInclusive
                            )
                      ...
                      {partsSubtotal > 0 && laborSubtotal > 0 && (
                        <>
                          <div className="flex justify-between text-sm">
                            ...
                            <span>
                              {formatCurrency(partsSubtotal, currencyCode, currencyFormat)}
                            </span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span className="text-gray-500">{t('labor')}</span>
                            <span>
                              {formatCurrency(laborSubtotal, currencyCode, currencyFormat)}
                            </span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span className="text-gray-500">{t('subtotal')}</span>
                            <span>
                              {formatCurrency(computedSubtotal, currencyCode, currencyFormat)}
                            </span>
                          </div>
                        </>
                      )}
                      {displayDiscountAmount > 0 && (
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-500">
                            {record.discountType === 'percentage'
                              ? t('discountPercent', { percent: record.discountValue })
                              : t('discount')}
                          </span>
              ...
              case 'general': {
                ...
                return (
                  <div key="general" className="mt-6 rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
                    <p className="mb-2 text-xs font-bold uppercase" style={{ color: primaryColor }}>
                      {t('customFields', { defaultValue: 'Additional Information' })}
                    </p>
              ...
              case 'notes':
                ...
                return (
                  <div key="notes" className="mt-6 rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
                    <p className="mb-1 text-xs font-bold uppercase" style={{ color: primaryColor }}>
                      {t('notes')}
                    </p>
                    <div
              ...
              case 'warranty': {
                ...
                if (record.warrantyMonths)
                  parts.push(
                    `${record.warrantyMonths} ${record.warrantyMonths === 1 ? t('warrantyMonth', { defaultValue: 'month' }) : t('warrantyMonths', { defaultValue: 'months' })}`
                  )
                if (record.warrantyMileage)
                  parts.push(
                    `${record.warrantyMileage.toLocaleString()} ${t('warrantyKm', { defaultValue: 'km' })}`
                  )
                const expiresAt = record.warrantyExpiresAt
                  ? fmtDate(new Date(record.warrantyExpiresAt), df)
                  : null
                return (
                  <div key="warranty" className="mt-4 rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
                    <p className="mb-1 text-xs font-bold uppercase" style={{ color: primaryColor }}>
                      {t('warrantyTitle', { defaultValue: 'Warranty' })}
                    </p>
                    {parts.length > 0 && (
                    ...
                    {expiresAt && (
                      <p className="text-sm text-gray-500 dark:text-gray-400">
                        {t('warrantyExpires', { defaultValue: 'Expires' })}: {expiresAt}
                      </p>
                    )}
                ...
                return (
                  <div key="findings" className="mt-6">
                    <h4 className="mb-1 font-semibold">
                      {t('findings', { defaultValue: 'Findings' })}
                    </h4>
                          ...
                          {findings.map((f, i) => {
                            const severityColor =
                              f.severity === 'urgent'
                                ...
                                : f.severity === 'needs_work'
                                  ? 'text-amber-600 dark:text-amber-400'
                                  : 'text-blue-600 dark:text-blue-400'
                            const severityLabel =
                              f.severity === 'urgent'
                                ? t('findingSeverityUrgent', { defaultValue: 'Urgent' })
                                : f.severity === 'needs_work'
                                  ? t('findingSeverityNeedsWork', { defaultValue: 'Needs Work' })
                                  : t('findingSeverityMonitor', { defaultValue: 'Monitor' })
                            return (
                              <tr key={i}>
                                <td className="p-2">
                                  <span
                                    className={`text-xs font-semibold uppercase ${severityColor}`}
                                  >
                                    {severityLabel}
                                  </span>
                                </td>
                      ...
                      {(() => {
                        const dueDate = record.invoiceDueDate
                          ...
                          : invoiceSettings.dueDays > 0
                            ? new Date(
                                new Date(effectiveInvoiceDate).getTime() +
                                  invoiceSettings.dueDays * 86400000
                              )
                            : null
                        const netDays = dueDate
                          ? Math.ceil(
                              (dueDate.getTime() - new Date(effectiveInvoiceDate).getTime()) /
                                86400000
                            )
                          : null
              ...
              case 'telegram_qr':
                ...
                return (
                  <div
                    key="telegram_qr"
                    className="mt-4 border-t pt-4 flex flex-col items-center gap-2"
                  >
