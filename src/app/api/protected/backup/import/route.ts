        ...
        for (const set of data.tireSets as Record<string, unknown>[]) {
          ...
          if (agreements?.length) {
            for (const ag of agreements) {
              // Every period is derived from the start date, so an agreement
              // without a readable one cannot be billed correctly. Dropped
              // rather than restored to an invented date, which would silently
              // shift every period under it.
              const startDate = toSafeDate(ag.startDate as string)
              if (!startDate) continue

              await tx.tireStorageAgreement.create({
                data: {
                  id: ag.id as string,
                  billingModel: (ag.billingModel as string) || 'seasonal',
                  price: (ag.price as number) || 0,
                  extras: (ag.extras as object) ?? undefined,
                  startDate,
                  endDate: ag.endDate ? toSafeDate(ag.endDate as string) : null,
                  autoRenew: (ag.autoRenew as boolean) || false,
                  status: (ag.status as string) || 'active',
                  notes: (ag.notes as string) || null,
                  createdAt: toSafeDate(ag.createdAt as string),
                  updatedAt: toSafeDate(ag.updatedAt as string),
                  tireSetId: set.id as string,
                  customerId: (ag.customerId as string) || null,
                  organizationId: ctx.organizationId,
                  userId: ctx.userId,
                },
              })

              const charges = ag.charges as Record<string, unknown>[] | undefined
              if (charges?.length) {
                await tx.tireStorageCharge.createMany({
                  // Same reasoning: a charge with no readable period is not a
                  // period anyone can bill or reconcile.
                  data: charges
                    .filter(
                      (ch) =>
                        !!toSafeDate(ch.periodStart as string) &&
                        !!toSafeDate(ch.periodEnd as string)
                    )
                    .map((ch) => ({
                      id: ch.id as string,
                      ...
                      agreementId: ag.id as string,
                      // The invoice it landed on is restored separately and may
                      // not be present. The status still records that it was
                      // billed, which is what stops a re-bill.
