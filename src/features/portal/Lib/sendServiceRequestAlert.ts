 ...
 */
export async function sendServiceRequestAlert(input: ServiceRequestAlertInput): Promise<number> {
  try {
    ...
    const recipients =
      configured.length > 0 ? configured : await getDefaultRecipients(input.organizationId)
    ...
    const html = buildServiceRequestEmailHtml({
      ...
      requestUrl: appUrl ? `${appUrl}/customers/${input.customerId}?tab=requests` : null,
    })
