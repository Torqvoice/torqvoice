...
export function AiSettingsForm({ initial }: { initial: Record<string, string> }) {
  ...
  const [enabled, setEnabled] = useState(initial[AI_KEYS.AI_ENABLED] === 'true' || false)
  const [provider, setProvider] = useState<AiProvider>(
    (initial[AI_KEYS.AI_PROVIDER] as AiProvider) || 'openai'
  )
  const [apiKey, setApiKey] = useState(initial[AI_KEYS.AI_API_KEY] || '')
  const [model, setModel] = useState(initial[AI_KEYS.AI_MODEL] || 'gpt-4o-mini')
        ...
        >
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">{t('ai.enableHint')}</p>
            </div>
          ...
          {!enabled && (
            <div className="flex items-start gap-3 rounded-lg border bg-muted/50 p-4">
              ...
              <p className="text-sm text-muted-foreground">{t('ai.disabledInfo')}</p>
            </div>
          )}
