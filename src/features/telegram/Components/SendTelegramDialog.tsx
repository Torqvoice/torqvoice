...
import { sendTelegramToCustomer } from "../Actions/telegramActions";
...
export function SendTelegramDialog({
  ...
  const t = useTranslations("telegramMessages.sendDialog");
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    const result = await sendTelegramToCustomer({
      customerId,
      body: message.trim(),
      relatedEntityType,
      relatedEntityId,
      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSend} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="telegram-message">{t("message")}</Label>
            <Textarea
              id="telegram-message"
