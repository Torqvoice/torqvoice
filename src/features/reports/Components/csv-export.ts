...
export function exportInventoryCsv(
  ...
  const rows = data.lowStock.map((p) => ({
    ...
    quantity: p.unit ? `${p.quantity} ${p.unit}` : p.quantity,
    minQuantity:
      p.minQuantity != null ? (p.unit ? `${p.minQuantity} ${p.unit}` : p.minQuantity) : '',
...
export function exportTechniciansCsv(
  ...
  downloadCsv('technicians-report.csv', headers, rows, [
    'techName',
    'jobCount',
    'totalRevenue',
    'avgRevenue',
...
export function exportPartsCsv(
  ...
  downloadCsv('parts-usage-report.csv', headers, rows, [
    'name',
    'partNumber',
    'usageCount',
    'totalQuantity',
...
export function exportRetentionCsv(
  ...
  downloadCsv('retention-report.csv', headers, rows, [
    'name',
    'company',
    'visitCount',
    'totalSpent',
...
export function exportPastDueInvoicesCsv(
  ...
  downloadCsv('past-due-invoices-report.csv', headers, rows, [
    'customer',
    'company',
    'invoiceNumber',
    'totalAmount',
...
export function exportVehicleReportCsv(
  ...
  downloadCsv(`vehicle-report-${vehicleLabel.replace(/\s+/g, '-')}.csv`, headers, rows, [
    'date',
    'title',
    'type',
    'status',
...
export function exportTaxCsv(
  ...
  downloadCsv('tax-report.csv', headers, rows, [
    'month',
    'taxCollected',
    'taxableAmount',
    'invoiceCount',
