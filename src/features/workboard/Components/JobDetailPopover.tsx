...
export function JobDetailPopover({
  ...
  const saveTimes = useCallback(
    async (start: Date | undefined, end: Date | undefined) => {
      ...
      store.updateServiceTimes(job.id, start.toISOString(), end.toISOString())
  ...
  const currentHours =
    startDate && endDate ? Math.round((endDate.getTime() - startDate.getTime()) / 3600000) : null
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="space-y-3">
          ...
          <DurationPicker value={currentHours} onChange={handleDurationChange} />
