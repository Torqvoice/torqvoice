...
export const MESSAGE_CHANNELS = ["email", "sms", "telegram", "in_app"] as const;
...
export const createScheduledMessageSchema = z
  ...
  .refine(
    (v) => v.channel === "in_app" || !!v.customerId || !!v.recipient?.trim(),
    { message: "Pick a customer or enter a recipient", path: ["recipient"] },
  );
