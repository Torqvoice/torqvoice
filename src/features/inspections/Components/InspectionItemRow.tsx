...
export function InspectionItemRow({
  ...
  const [measured, setMeasured] = useState(
    item.measuredValue === null || item.measuredValue === undefined ? "" : String(item.measuredValue)
  );
    ...
    >
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 flex-1">
          <p id={nameId} className="flex flex-wrap items-baseline gap-x-2 gap-y-1 text-sm font-medium">
            {item.code && (
      ...
      {inputType === "text" && (
        <div className="mt-3 space-y-1">
          ...
          <Input
            ...
            onBlur={() => !isCompleted && textValue !== (item.textValue ?? "") && save({ textValue })}
            disabled={isCompleted}
            className="h-11"
          />
        </div>
