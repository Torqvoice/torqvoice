...
function parseCSV(text: string): ParsedRow[] {
  ...
  for (let i = 0; i < headers.length; i++) {
    const h = headers[i]
    if (
      ['name', 'fullname', 'contactname', 'customername', 'displayname', 'firstname'].includes(h)
    ) {
  ...
  const lastNameIdx = headers.findIndex((h) =>
    ['lastname', 'surname', 'familyname', 'last'].includes(h)
  )
  ...
  if (
    !headerMap[
      String(
        headers.findIndex((h) =>
          ['name', 'fullname', 'contactname', 'customername', 'displayname'].includes(h)
        )
      )
    ] &&
    !hasFirstLast
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-7xl max-h-[85vh] !overflow-hidden">
        <DialogHeader>
          ...
          <DocsLink
            href="/docs/tutorials/migrate-customers"
            variant="hint"
          ...
          <>
            <div className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-2 flex-wrap text-muted-foreground">
                <span>
                  {fileName} — {t('rowsFound', { count: rows.length })}
                </span>
                {dupCount > 0 && (
                  <Button
                    ...
                    className={`h-7 text-xs gap-1.5 ${
                      !showDupsOnly
                        ? 'text-amber-600 border-amber-500/40 hover:bg-amber-50 dark:hover:bg-amber-950/30'
                        : ''
                    }`}
                    ...
                    <TableBody>
                      {rows.map((row, i) => {
                        ...
                        return (
                          <TableRow
                            key={i}
                            className={dup ? (action === 'skip' ? 'opacity-50' : '') : ''}
                          >
                              ...
                              {dup ? (
                                ...
                                >
                                  {action === 'skip' ? (
                                    <Badge
                                      variant="outline"
                                      className="text-muted-foreground cursor-pointer hover:border-foreground/40 text-xs gap-1"
                                    >
                                      <SkipForward className="h-3 w-3" />
                                      {t('actionSkip')}
                                    </Badge>
                                    ...
                                    <Badge
                                      variant="outline"
                                      className="text-blue-600 border-blue-500/30 cursor-pointer hover:border-blue-500/60 text-xs gap-1"
                                    >
                                      <Merge className="h-3 w-3" />
                                      {t('actionMerge')}
                                    </Badge>
            ...
            <div className="flex items-center justify-between pt-2">
              <div className="text-xs text-muted-foreground">
                ...
                {mergeCount > 0 && (
                  <span>
                    {newCount > 0 ? ' · ' : ''}
                    {t('summaryMerge', { count: mergeCount })}
                  </span>
                )}
                {skipCount > 0 && (
                  <span>
                    {newCount > 0 || mergeCount > 0 ? ' · ' : ''}
                    {t('summarySkip', { count: skipCount })}
                  </span>
                )}
              </div>
              <div className="flex gap-3">
                ...
                <Button
                  ...
                  disabled={importing || (newCount === 0 && mergeCount === 0)}
                >
