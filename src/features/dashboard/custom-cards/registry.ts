...
export function sanitizeConfig(raw: unknown): CustomCardConfig | null {
  ...
  const columns = Array.isArray(c.columns)
    ? (c.columns.filter((col): col is string => typeof col === "string" && fieldIds.has(col)).slice(0, 8))
    : [];
  ...
  if (Array.isArray(c.filters)) {
    for (const rawFilter of c.filters.slice(0, 8)) {
      ...
      if (typeof field !== "string" || typeof operator !== "string" || typeof value !== "string") continue;
  ...
  const limit = typeof c.limit === "number" && Number.isFinite(c.limit)
