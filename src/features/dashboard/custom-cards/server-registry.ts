...
function condFor(entity: CardEntity, filter: CardFilter): Cond | null {
  ...
  if (!def) return null;
  switch (def.type) {
    case "text": return textCond(filter.operator, filter.value);
    case "number": return numberCond(filter.operator, filter.value);
    case "date": return dateCond(filter.operator, filter.value);
    case "select":
  ...
  switch (config.entity) {
    case "vehicles": {
      const rows = await db.vehicle.findMany({
        ...
        select: {
          id: true, licensePlate: true, make: true, model: true, year: true,
          color: true, vin: true, mileage: true, fuelType: true,
          transmission: true, createdAt: true, updatedAt: true,
      ...
      return rows.map((r) => ({
        ...
        href: `/vehicles/${r.id}`,
        cells: {
          licensePlate: r.licensePlate, make: r.make, model: r.model,
          year: r.year, color: r.color, vin: r.vin, mileage: r.mileage,
    ...
    case "customers": {
      const rows = await db.customer.findMany({
        ...
        select: {
          ...
          address: true, taxId: true, createdAt: true,
        },
        orderBy: { updatedAt: "desc" },
        take: limit,
      });
      return rows.map((r) => ({
        ...
        cells: {
          ...
          address: r.address, taxId: r.taxId, createdAt: iso(r.createdAt),
        },
      }));
    }
    case "workOrders": {
    ...
    case "quotes": {
      const rows = await db.quote.findMany({
        ...
        select: {
          ...
          totalAmount: true, validUntil: true, createdAt: true,
          customer: { select: { name: true } },
      ...
      return rows.map((r) => ({
        ...
        href: `/quotes/${r.id}`,
        cells: {
          quoteNumber: r.quoteNumber, title: r.title, status: r.status,
          totalAmount: r.totalAmount, validUntil: iso(r.validUntil),
    ...
    case "inventory": {
      const rows = await db.inventoryPart.findMany({
        where: buildWhere(config, { organizationId }),
        select: {
          id: true, name: true, partNumber: true, category: true,
          quantity: true, minQuantity: true, unitCost: true, sellPrice: true,
          supplier: true,
        },
        orderBy: { updatedAt: "desc" },
        take: limit,
      ...
      return rows.map((r) => ({
        ...
        href: `/inventory/${r.id}`,
        cells: {
          name: r.name, partNumber: r.partNumber, category: r.category,
          quantity: r.quantity, minQuantity: r.minQuantity,
          unitCost: r.unitCost, sellPrice: r.sellPrice, supplier: r.supplier,
        },
      }));
    }
  }
