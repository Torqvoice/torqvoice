                ...
                {isExpanded && hasDetails && (
                  <div className="px-3 pb-2 space-y-0.5">
                    {preset.items.map((item, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between text-xs text-muted-foreground py-0.5 pl-2 border-l-2 border-muted"
                      >
                        <span className="truncate">{item.description}</span>
                        <span className="shrink-0 ml-2">
                          {item.pricingType === 'service'
                            ? `${item.hours} unit${item.hours !== 1 ? 's' : ''}`
                            : `${item.hours}h`}
                        </span>
                      </div>
                    ))}
                    {preset.parts && preset.parts.length > 0 && (
                      <>
                        {preset.parts.map((part, i) => (
                          <div
                            key={`part-${i}`}
                            className="flex items-center justify-between text-xs text-muted-foreground py-0.5 pl-2 border-l-2 border-blue-400/50"
                          >
