...
import {
  ...
  type TelegramThread,
} from '@/features/telegram/Components/TelegramMessagesClient'
...
type Tab = 'inbox' | 'whatsapp' | 'telegram' | 'scheduled'
...
export function MessagesPageClient({
  ...
  /** Conversations on the other channels, loaded on the server with the SMS ones. */
  whatsappEnabled?: boolean
  whatsappThreads?: WhatsappThreadSummary[]
  telegramEnabled?: boolean
  telegramThreads?: TelegramThread[]
  telegramHasMore?: boolean
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-1 rounded-lg border p-1">
          ...
          {whatsappEnabled && (
            <Button
              size="sm"
              variant={tab === 'whatsapp' ? 'default' : 'ghost'}
              className="h-8"
          ...
          {telegramEnabled && (
            <Button
              size="sm"
              variant={tab === 'telegram' ? 'default' : 'ghost'}
              className="h-8"
      ...
      <div className="flex h-[calc(100vh-11rem)] overflow-hidden rounded-lg border bg-background">
        {tab === 'whatsapp' ? (
          <WhatsappMessagesClient threads={whatsappThreads} />
        ) : tab === 'telegram' ? (
          <TelegramMessagesClient
            initialThreads={telegramThreads}
            initialHasMore={telegramHasMore}
          />
        ) : tab === 'scheduled' ? (
